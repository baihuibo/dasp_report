...
app.factory('captures', function (webCaptureProxy, $q, webLogs, date, writeFile, fs) {
    ...
    function proxy(path, file, promises, obj) {
    ...
    return {
        dljr: function (obj) {
            ...
            //邮政储蓄银行网点
            proxy(path, `CUST_BANK_DAY_${time}.csv`, ps1, {web, action: dljr(rpa1), time, hds: hds1});
            //邮政储蓄代理网点
            proxy(path, `CUST_PROXY_DAY_${time}.csv`, ps1, {web, action: dljr(rpa2), time, hds: hds1});

            //去年
            //邮政储蓄银行网点
            proxy(path, `CUST_BANK_DAY_SAME_${time}.csv`, ps1, {web, action: dljr(rpa1, 1), time, hds: hds1});
            //邮政储蓄代理网点
            proxy(path, `CUST_PROXY_DAY_SAME_${time}.csv`, ps1, {web, action: dljr(rpa2, 1), time, hds: hds1});

            return $q.all(ps1).then(function () {
                //中国邮政储蓄存款余额分种类统计表-日报
                var ps2 = [];
                proxy(path, `CUST_DIV_DAY_BANK_${time}.csv`, ps2, {web, action: dljr2(rpa1), time, hds: hds2});
                proxy(path, `CUST_DIV_DAY_TOTAL_${time}.csv`, ps2, {web, action: dljr2(rpa2), time, hds: hds2});

                //去年
                proxy(path, `CUST_DIV_DAY_BANK_SAME_${time}.csv`, ps2, {web, action: dljr2(rpa1, 1), time, hds: hds2});
                proxy(path, `CUST_DIV_DAY_TOTAL_SAME_${time}.csv`, ps2, {web, action: dljr2(rpa2, 1), time, hds: hds2});
                return $q.all(ps2);
            });
